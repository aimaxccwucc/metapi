...
vi.mock('../../services/notifyService.js', async () => {
  const actual = await vi.importActual<typeof import('../../services/notifyService.js')>('../../services/notifyService.js');
  return {
    ...actual,
...
describe('settings custom routes', () => {
  ...
  beforeAll(async () => {
    dataDir = mkdtempSync(join(tmpdir(), 'metapi-settings-custom-routes-'));
    process.env.DATA_DIR = dataDir;

    await import('../../db/migrate.js');
    const dbModule = await import('../../db/index.js');
    const routesModule = await import('./settingsCustom.js');

    db = dbModule.db;
    schema = dbModule.schema;

    app = Fastify();
    await app.register(routesModule.registerSettingsCustomRoutes);
  });
  ...
  it('sends test notification through custom settings route', async () => {
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/api/settings/notify/test',
    });

    expect(response.statusCode).toBe(200);
    const body = response.json() as { success: boolean; message: string };
    expect(body.success).toBe(true);
    expect(body.message).toBe('测试通知已发送（成功 2/2）');
  ...
  it('returns validation error when test notification fails', async () => {
    sendNotificationMock.mockRejectedValue(new Error('未启用任何通知渠道，请先开启并保存至少一种通知方式'));
    ...
    expect(body.message).toBe('未启用任何通知渠道，请先开启并保存至少一种通知方式');
  });
});
