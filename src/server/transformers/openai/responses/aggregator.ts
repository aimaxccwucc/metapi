...
function ensureMessageItem(state: OpenAiResponsesAggregateState, indexHint?: number): { index: number; item: AggregateOutputItem } {
  let index = state.messageIndex ?? indexHint ?? 0;
  // If the target index is already occupied by a non-message item (e.g. reasoning),
  // skip to the next available index so the message gets its own output slot.
  const existing = state.outputItems[index];
  if (existing && typeof existing.type === 'string' && existing.type !== 'message') {
