...
describe('chat proxy stream behavior', () => {
  ...
  it('returns upstream_error for empty anthropic messages payloads routed from chat completions', async () => {
    config.proxyEmptyContentFailEnabled = true;
  ...
  it('does not record success when a native chat SSE stream completes without meaningful output and empty-content failure is enabled', async () => {
    config.proxyEmptyContentFailEnabled = true;

    const encoder = new TextEncoder();
    const upstreamBody = new ReadableStream<Uint8Array>({
      start(controller) {
        controller.enqueue(encoder.encode('data: {"id":"chatcmpl-empty-sse","model":"upstream-gpt","choices":[{"delta":{"role":"assistant"},"finish_reason":null}]}\n\n'));
  ...
  it('normalizes fc-style tool ids during codex chat continuation across requests', async () => {
    fetchMock
      .mockResolvedValueOnce(new Response(JSON.stringify({
        id: 'chatcmpl-tool-step-fc-1',
        object: 'chat.completion',
        model: 'gpt-5.4',
        choices: [{
          index: 0,
    ...
    const headers = {
      Originator: 'codex_cli_rs',
      Session_id: 'codex-session-memory-fc-1',
    };
    const tools = [{
      type: 'function',
      function: {
    ...
    const secondResponse = await app.inject({
      ...
      payload: {
        ...
        messages: [
          { role: 'user', content: 'call tool' },
          { role: 'tool', id: 'fc_mem_fc_1', content: '{"temp":"22C"}' },
        ],
        tools,
      },
    });
    ...
    expect(forwardedBody.messages).toEqual([
      ...
      { role: 'tool', id: 'fc_mem_fc_1', tool_call_id: 'call_mem_fc_1', content: '{"temp":"22C"}' },
    ]);
  });

  it('keeps codex standalone tool continuation working with fast-fail routing enabled', async () => {
    fetchMock
      .mockResolvedValueOnce(new Response(JSON.stringify({
        id: 'chatcmpl-tool-timeout-1',
        object: 'chat.completion',
        created: 1_706_000_000,
        model: 'upstream-gpt',
        choices: [{
    ...
    const headers = {
      Originator: 'codex_cli_rs',
      Session_id: 'codex-session-timeout-1',
    };
    const tools = [{
      type: 'function',
      function: {
    ...
    const secondResponse = await app.inject({
      ...
      payload: {
        ...
        messages: [
          { role: 'user', content: 'call tool' },
          { role: 'tool', tool_call_id: 'call_fast_fail_1', content: '{"temp":"22C"}' },
        ],
        tools,
      },
    });
