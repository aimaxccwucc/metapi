/**
 * Shared surface helpers for proxy request handling.
 *
 * Adapted from upstream `sharedSurface.ts` to work with our codebase layout.
 * Key differences from upstream:
 *   - Import paths remapped (we keep route-level modules under routes/proxy/)
 *   - `proxyChannelCoordinator` is a stub (our conductor handles sticky sessions)
 *   - `recordOauthQuotaHeadersSnapshot` is a stub
 *   - `channelSelection` is a simplified version without forced-channel support
 */

import { formatUtcSqlDateTime } from '../../services/localTimeService.js';
import { resolveChannelProxyUrl, withSiteRecordProxyRequestInit } from '../../services/siteProxy.js';
// SiteProxyConfigLike is not exported from our siteProxy module;
// inline the subset we need for dispatch.
...
export async function recordSurfaceSuccess(input: {
  ...
  try {
    const fallbackResult = await resolveProxyUsageWithSelfLogFallback({
    ...
    // Adapt ProxyUsageFallbackResult (no usageSource) to SurfaceResolvedUsageSummary
    resolvedUsage = {
      ...fallbackResult,
      usageSource: hasUpstreamUsage ? 'upstream' : (fallbackResult.recoveredFromSelfLog ? 'self-log' : 'unknown'),
    };
