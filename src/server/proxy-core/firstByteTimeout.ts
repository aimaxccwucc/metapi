...
const observedResponseMeta = new WeakMap<UndiciResponse, ObservedResponseMeta>();

function setObservedResponseMeta<T extends UndiciResponse>(response: T, meta: ObservedResponseMeta): T {
...
function buildObservedTimeoutResponse(timeoutMs: number): UndiciResponse {
  return setObservedResponseMeta(new globalThis.Response(buildFirstByteTimeoutMessage(timeoutMs), {
    status: 408,
...
function buildReplayResponse<T extends UndiciResponse>(
  response: T,
  ...
  return new globalThis.Response(stream as any, {
...
export async function fetchWithObservedFirstByte<T extends UndiciResponse>(
...
export function getObservedResponseMeta(response: UndiciResponse | null | undefined): ObservedResponseMeta | null {
  if (!response) return null;
  return observedResponseMeta.get(response) ?? null;
}

export function isObservedFirstByteTimeoutResponse(response: UndiciResponse | null | undefined): boolean {
