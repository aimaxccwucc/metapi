...
export const tokenRoutes = sqliteTable('token_routes', {
  ...
  routingStrategy: text('routing_strategy').default('stable_first'),
  enabled: integer('enabled', { mode: 'boolean' }).default(true),
  createdAt: text('created_at').default(sql`(datetime('now'))`),
  updatedAt: text('updated_at').default(sql`(datetime('now'))`),
}, (table) => ({
...
export const proxyLogs = sqliteTable('proxy_logs', {
  ...
  cacheSavedCost: real('cache_saved_cost'),
...
export const downstreamApiKeys = sqliteTable('downstream_api_keys', {
  ...
  excludedCredentialRefs: text('excluded_credential_refs'), // JSON array<string>
  lastUsedAt: text('last_used_at'),
  createdAt: text('created_at').default(sql`(datetime('now'))`),
  updatedAt: text('updated_at').default(sql`(datetime('now'))`),
}, (table) => ({
