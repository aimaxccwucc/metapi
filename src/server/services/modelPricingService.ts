...
export type PreferredTokenGroupResolution = {
  group: string;
...
function normalizeAvailableGroups(groups?: string[] | null): string[] {
  const deduped = new Map<string, string>();
  for (const rawGroup of groups || []) {
    const group = String(rawGroup || '').trim();
    if (!group) continue;
    const groupKey = group.toLowerCase();
    if (!deduped.has(groupKey)) deduped.set(groupKey, group);
  }
  return deduped.size > 0 ? Array.from(deduped.values()) : [DEFAULT_GROUP];
}

function getCaseInsensitiveGroupRatio(group: string, groupRatio?: Record<string, number> | null): number | null {
  if (!groupRatio || typeof groupRatio !== 'object') return null;
  const target = group.trim().toLowerCase();
  if (!target) return null;
  for (const [key, value] of Object.entries(groupRatio)) {
...
export function buildAvailableGroupRatios(
  availableGroups: string[],
  groupRatio?: Record<string, number> | null,
  fallbackToDefault = true,
): Record<string, number> {
  const normalizedGroups = normalizeAvailableGroups(availableGroups);
  const defaultRatio = fallbackToDefault
    ? (getCaseInsensitiveGroupRatio(DEFAULT_GROUP, groupRatio) ?? 1)
    : null;

  return normalizedGroups.reduce<Record<string, number>>((acc, group) => {
    const ratio = getCaseInsensitiveGroupRatio(group, groupRatio);
    if (ratio != null) {
      acc[group] = ratio;
      return acc;
    }
    if (defaultRatio != null) {
      acc[group] = defaultRatio;
    }
    return acc;
  }, {});
}

function findCatalogModelEntry(catalog: ModelPricingCatalog, modelName: string): ModelPricingCatalogEntry | null {
  const target = modelName.trim().toLowerCase();
  if (!target) return null;
  return catalog.models.find((item) => item.modelName.trim().toLowerCase() === target) || null;
}

function modelSupportsGroup(model: ModelPricingCatalogEntry, group: string): boolean {
  const normalizedGroup = group.trim().toLowerCase();
  if (!normalizedGroup) return false;
  const coverage = new Set<string>();
  for (const rawGroup of [...Object.keys(model.groupPricing || {}), ...(model.enableGroups || [])]) {
    const current = String(rawGroup || '').trim().toLowerCase();
    if (current) coverage.add(current);
  }
  if (coverage.size === 0) coverage.add(DEFAULT_GROUP);
  return coverage.has(normalizedGroup);
}

function pickLowestRatioGroup(groups: string[], groupRatios: Record<string, number>): string {
  const normalizedGroups = normalizeAvailableGroups(groups);
  return [...normalizedGroups].sort((left, right) => {
    const leftRatio = groupRatios[left] ?? Number.POSITIVE_INFINITY;
    const rightRatio = groupRatios[right] ?? Number.POSITIVE_INFINITY;
    if (leftRatio !== rightRatio) return leftRatio - rightRatio;
    return left.localeCompare(right);
  })[0] || DEFAULT_GROUP;
}

export function selectPreferredTokenGroup(options: {
  availableGroups?: string[] | null;
  modelNames?: string[] | null;
  catalog?: ModelPricingCatalog | null;
  fallbackToDefaultRatio?: boolean;
}): Omit<PreferredTokenGroupResolution, 'catalog'> {
  const availableGroups = normalizeAvailableGroups(options.availableGroups);
  const groupRatios = buildAvailableGroupRatios(
    availableGroups,
    options.catalog?.groupRatio,
    options.fallbackToDefaultRatio ?? true,
  );

  let candidateGroups = availableGroups;
  ...
  if (options.catalog && requestedModels.length > 0) {
    const matchedModels = requestedModels
      .map((modelName) => findCatalogModelEntry(options.catalog!, modelName))
      .filter((item): item is ModelPricingCatalogEntry => !!item);
    if (matchedModels.length === requestedModels.length && matchedModels.length > 0) {
      const fullyCoveredGroups = availableGroups.filter((group) => matchedModels.every((model) => modelSupportsGroup(model, group)));
      if (fullyCoveredGroups.length > 0) {
        candidateGroups = fullyCoveredGroups;
      }
    }
  }

  return {
    group: pickLowestRatioGroup(candidateGroups, groupRatios),
    availableGroups,
    candidateGroups,
    groupRatios,
  };
...
export async function resolvePreferredTokenGroup(
  input: EstimateProxyCostInput & {
    availableGroups?: string[] | null;
    modelNames?: string[] | null;
  },
): Promise<PreferredTokenGroupResolution> {
  let catalog: ModelPricingCatalog | null = null;
  try {
    catalog = await fetchModelPricingCatalog(input);
  } catch {
    catalog = null;
  }
