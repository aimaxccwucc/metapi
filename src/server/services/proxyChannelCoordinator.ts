...
function isSessionScopedChannel(input?: SessionScopedChannelInput): boolean {
  if (getCredentialModeFromExtraConfig(getSessionScopedExtraConfig(input)) === 'session') return true;
  if (typeof input === 'object' && input != null && input.oauthProvider) return true;
  return hasOauthProvider(getSessionScopedExtraConfig(input));
}
