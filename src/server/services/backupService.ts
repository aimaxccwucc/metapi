...
export interface AllApiHubMergeImportResult {
  importedRows: number;
  skippedRows: number;
  sites: {
    created: number;
...
function resolveCredentialModeForLegacyImport(
  authType: string,
  accessToken: string,
...
function collectLegacyKeyCandidates(
  ...
  await db.transaction(async (tx) => {
    ...
    for (const row of section.sites) {
      await tx.insert(schema.sites).values({
        ...
        flaresolverrUrl: row.flaresolverrUrl ?? null,
...
export async function importAllApiHubAccountsMerge(data: RawBackupData): Promise<AllApiHubMergeImportResult> {
  ...
  const [existingSites, existingAccounts, existingTokens] = await Promise.all([
    db.select().from(schema.sites).orderBy(asc(schema.sites.id)).all(),
    db.select().from(schema.accounts).orderBy(asc(schema.accounts.id)).all(),
    db.select().from(schema.accountTokens).orderBy(asc(schema.accountTokens.id)).all(),
  ]);
  ...
  await db.transaction(async (tx) => {
    for (const item of rows) {
      ...
      const existingSite = siteByKey.get(siteKey);
      let site: SiteRow;
      if (!existingSite) {
        const now = new Date().toISOString();
        const createdSite = await tx.insert(schema.sites).values({
          name: siteName,
          url: siteUrl,
