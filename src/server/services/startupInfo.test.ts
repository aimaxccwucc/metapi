...
describe('startupInfo', () => {
  it('builds single-port endpoint summary for admin and proxy APIs', () => {
    ...
    expect(endpoints.healthzUrl).toBe('http://127.0.0.1:4000/healthz');
    expect(endpoints.readyzUrl).toBe('http://127.0.0.1:4000/readyz');
    expect(endpoints.metricsUrl).toBe('http://127.0.0.1:4000/metrics');
  });
  ...
  it('renders copy-ready startup summary lines', () => {
    ...
    expect(lines.some((line) => line.includes('/healthz'))).toBe(true);
    expect(lines.some((line) => line.includes('/readyz'))).toBe(true);
    expect(lines.some((line) => line.includes('/metrics'))).toBe(true);
    expect(lines.some((line) => line.includes('Admin auth:'))).toBe(true);
    expect(lines.some((line) => line.includes('Proxy auth:'))).toBe(true);
  });
});
