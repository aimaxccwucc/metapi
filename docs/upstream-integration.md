...
当前仓库已经把“上游接入”和“稳定性治理”分开建模：

- 本文档负责说明站点怎么接入、凭证怎么填、各平台有哪些已知接法。
- 路由稳定性、协议有限探测、签到状态机和后续改造路线，统一见 [路由与签到稳定性改造](./gateway-checkin-hardening.md)。

特别注意两条约束：

- 协议探测必须是“单模型有限验证”，不能自动探测一个站点的全部模型。
- 站点协议配置应优先来源于“已验证成功的端点”，不能把默认顺序直接当成支持列表。
...
如果某个站点协议特征复杂、模型兼容性分裂、或存在强风控页面，请优先参考 [路由与签到稳定性改造](./gateway-checkin-hardening.md) 中的 `site type profile / protocol capability profile` 思路，而不是继续扩展“全站自动猜测”。
