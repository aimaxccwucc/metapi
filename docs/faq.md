...
- 如果你在非 Compose 场景未显式设置 `AUTH_TOKEN`，服务会在首次启动时自动生成随机管理员令牌并写入当前运行数据库
...
如果你看到的是“总要先失败一次再成功”或“明明恢复了却长期不再被选中”，问题通常不在模型本身，而在协议记忆、端点回退或历史失败记忆。此时应参考：
...
- 站点开启了 Turnstile / 人工验证，此时不应被视为真正成功

如果你关心的是“当天必须至少成功一次”，而不是“尝试过一次就算处理完”，请同时参考：
...
### Q: 为什么不建议自动探测一个站点的全部模型？

**A:** 原因有三个：

- 风控风险高，容易触发上游限流、封禁或额外验证
- 得到的“全量列表”不一定等于“真实稳定可调用能力”
- 当前更需要的是“找出一个可稳定调用的协议和测试模型”，而不是做模型爬取

当前建议始终采用“单模型有限验证 + 已验证端点持久化”的方案。详见：
...
1. **应用内导入导出**（推荐）：在管理后台 → 导入/导出 页面操作，适合 SQLite / MySQL / Postgres 之间迁移
2. **目录迁移**：仅适用于当前运行在 SQLite 的实例
